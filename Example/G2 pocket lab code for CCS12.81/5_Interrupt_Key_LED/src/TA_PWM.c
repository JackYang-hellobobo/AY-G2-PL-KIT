...
char TA0_PWM_SetPermill(char Channel, unsigned int Duty)
{
    ...
    {
    case 1:
        Mod = (TA0CCTL1 & 0x00e0) >> 5;
        break;	//读取输出模式，OUTMOD0位于5-7位
    case 2:
        Mod = (TA0CCTL2 & 0x00e0) >> 5;
        break;	//读取输出模式，OUTMOD1位于5-7位
    default:
        return (0);
    }
    ...
    case 3:		//占空比一律为正脉宽，所以需要 TA0CCR0减去占空比
    {
        if (Percent > 1000)
            Percent = 1000;
        if (Channel == 1)
            TA0CCR1 = TA0CCR0 - Percent * TA0CCR0 / 1000;
        if (Channel == 2)
            TA0CCR2 = TA0CCR0 - Percent * TA0CCR0 / 1000;
        break;
    }
    default:
        return (0);
...
char TA1_PWM_SetPermill(char Channel, unsigned int Duty)
{
    ...
    {
    case 1:
        Mod = (TA1CCTL1 & 0x00e0) >> 5;
        break;	//读取输出模式，OUTMOD0位于5-7位
    case 2:
        Mod = (TA1CCTL2 & 0x00e0) >> 5;
        break;	//读取输出模式，OUTMOD1位于5-7位
    default:
        return (0);
    }
    ...
    {
    case 2:
    case 6: /**死区模式2,6时，需要判断修正死区时间，且同时设定TA1CCR1/2 的值*/
    {
    ...
    case 3:		//占空比一律为正脉宽，所以需要 TA1CCR0减去占空比
    {
        if (Percent > 1000)
            Percent = 1000;
        if (Channel == 1)
            TA1CCR1 = TA1CCR0 - Percent * TA1CCR0 / 1000;
        if (Channel == 2)
            TA1CCR2 = TA1CCR0 - Percent * TA1CCR0 / 1000;
        break;
    }
    default:
        return (0);
