...
_TI_PROPRIETARY_PRAGMA("CHECK_MISRA(\"-12.7\")") /* bitwise operators not allowed on signed ints */
_TI_PROPRIETARY_PRAGMA("CHECK_MISRA(\"-16.4\")") /* identifiers in fn defn/decl identical??? fabs/fabsf */
_TI_PROPRIETARY_PRAGMA("CHECK_MISRA(\"-19.1\")") /* only comments and preproc before #include */
_TI_PROPRIETARY_PRAGMA("CHECK_MISRA(\"-19.4\")") /* macros expand to simple things */
_TI_PROPRIETARY_PRAGMA("CHECK_MISRA(\"-19.7\")") /* macros required for implementation */
_TI_PROPRIETARY_PRAGMA("CHECK_MISRA(\"-19.10\")") /* macro params enclosed in parens */
...
#if (defined(_STDC_VERSION) && __STDC_VERSION__ >= 201112L)
...
#elif (defined(_STDC_VERSION) && (__STDC_VERSION__ >= 199901L) && defined(__EDG_VERSION__))
...
/*                                                                           */
/* This is done by defining sqrt to __relaxed_sqrt to allow other            */
/* translation units to use the normal sqrt routine under strict mode.       */
/*---------------------------------------------------------------------------*/
#if !__TI_PROPRIETARY_STRICT_FP_MACRO
#if __TI_HAS_BUILTIN_SQRT32
_TI_PROPRIETARY_PRAGMA("CHECK_MISRA(\"-19.4\")") /* macro defined to be an identifier is OK */
_TI_PROPRIETARY_PRAGMA("CHECK_MISRA(\"-8.1\")") /* function definition with no prototype visible */
_TI_PROPRIETARY_PRAGMA("CHECK_MISRA(\"-8.5\")") /* function definitions required */
_TI_PROPRIETARY_PRAGMA("CHECK_MISRA(\"-8.11\")") /* non-static function definition OK */
...
/*---------------------------------------------------------------------------*/
/* If --fp_mode=relaxed is used and C28x FPU is enabled, use the hardware    */
/* MAXF32 and MINF32 directly instead of calling fmaxf and fminf.            */
/*                                                                           */
/* This is done by defining fmax to __relaxed_fmax to allow other            */
/* translation units to use the normal fmax routine under strict mode.       */
/*---------------------------------------------------------------------------*/
#if !__TI_PROPRIETARY_STRICT_FP_MACRO
...
#define fmaxf __relaxed_fmaxf
__inline float __relaxed_fmaxf(float x, float y)
{
    return __fmax(x, y);
}

#define fminf __relaxed_fminf
__inline float __relaxed_fminf(float x, float y)
{
    return __fmin(x, y);
}
#endif
...
#endif
#endif /* defined(__TMS320C28XX_TMU__)  && !__TI_PROPRIETARY_STRICT_FP_MACRO */
